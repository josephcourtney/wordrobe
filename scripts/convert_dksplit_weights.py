"""Convert DKSplit's published ONNX/CRF files into Wordrobe's runtime format."""
...
def _required(initializers: dict[str, np.ndarray], name: str) -> np.ndarray:
    try:
        ...
        msg = f"required ONNX initializer is missing: {name}"
...
def convert(onnx_path: Path, crf_path: Path, output_path: Path) -> None:
    ...
    if len(lstm_nodes) != EXPECTED_LSTM_NODES:
        msg = f"expected exactly {EXPECTED_LSTM_NODES} DynamicQuantizeLSTM nodes, found {len(lstm_nodes)}"
        raise ValueError(msg)

    arrays: dict[str, np.ndarray] = {
        "format_version": np.asarray(FORMAT_VERSION, dtype=np.int32),
        "source": np.asarray("DKSplit 1.0.2 / ABTdomain / CC BY 4.0"),
    ...
    for layer, node in enumerate(lstm_nodes):
        if len(node.input) < MIN_LSTM_INPUTS:
            msg = f"layer {layer} has unexpected DynamicQuantizeLSTM inputs"
            raise ValueError(msg)
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT, help="destination converted NPZ")
    return parser
...
def main() -> None:
    ...
    if args.onnx is None and args.crf is None:
        ...
        msg = "--onnx and --crf must be supplied together"
