...
- Viterbi segmentation now carries minimal local sequence state: adjacent implicit singleton words are penalized and articles receive a modest bonus before an OOV noun, improving outputs such as `is this a snorql` while preserving identifier-like `openai`.
